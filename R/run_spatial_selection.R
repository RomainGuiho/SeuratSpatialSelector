...
#'
#' @importFrom Seurat ImageDimPlot CellsByIdentities
...
#' @param fovs Character vector of FOV identifiers to analyze (must match the values in `meta_id`)
#' @param images Optional character vector of image names (same length as `fovs`). If provided,
#'               `images[i]` is used as `fov` argument for the drawing function for `fovs[i]`.
#'               If NULL (default), no `fov` argument is provided to ImageDimPlot.
...
#' @param cols Optional color vector passed to ImageDimPlot.
#'
...
run_spatial_selection <- function(seurat_obj,
  ...
  if (missing(meta_id) || length(meta_id) != 1) stop("meta_id must be a single column name in seurat_obj@meta.data.")
  if (missing(fovs) || length(fovs) < 1) stop("Provide at least one fov in 'fovs'.")
  if (!is.null(images) && length(images) != length(fovs)) {
    stop("If 'images' is provided, it must have the same length as 'fovs'.")
  }

  # ---- Initialize metadata column safely ----
  if (!(metadata_col %in% colnames(seurat_obj@meta.data))) {
    ...
    # ensure same length
    if (length(seurat_obj@meta.data[[metadata_col]]) != ncol(seurat_obj)) {
  ...
  for (i in seq_along(fovs)) {
    fov <- fovs[i]
    message("Processing FOV: ", fov)

    # find cells for this fov using meta_id column
    ...
    selectedPoints <- tryCatch(
      ...
      error = function(e) {
        warning("Interactive selection failed for FOV ", fov, ": ", conditionMessage(e))
        return(NULL)
      }
    )
    if (is.null(selectedPoints) || nrow(selectedPoints) == 0) {
      message("No points selected for FOV: ", fov)
      next
    }

    # ---- Map selected points back to cell names ----
    plot_data <- subset_plot$data
    # merge by x,y (coordinates); use left_join if dplyr is available to be robust
    merged_selected <- tryCatch(
      merge(plot_data, selectedPoints, by = c("x", "y")),
      error = function(e) {
        warning("Merge of plot data and selected points failed for FOV ", fov, ": ", conditionMessage(e))
        return(NULL)
      }
    )
    ...
    # ensure column exists and initialized (again, safety)
    if (!(metadata_col %in% colnames(seurat_obj@meta.data))) {
      seurat_obj@meta.data[[metadata_col]] <- rep(negative_label, ncol(seurat_obj))
    }
    # set selected
